...
相关文件：`phase3_day3_tatoeba_ingest.py`（采集）、`phase3_day4_5_clean_validate.py`（清洗+校验）、`phase3_vectorize.py`（向量化）、`phase3_day6_7_faiss_search.py`（语义检索）、`airflow/dags/phase3_day8_airflow_dvc.py`（调度 DAG）
...
```
.
├── phase3_day3_tatoeba_ingest.py     # RAG：语料采集（分层 raw）
├── phase3_day4_5_clean_validate.py   # RAG：PySpark 清洗 + GX 校验
├── phase3_vectorize.py               # RAG：本地模型向量化（幂等）
├── phase3_day6_7_faiss_search.py     # RAG：FAISS 语义检索
...
# —— 项目一：RAG 语义检索 ——
python phase3_day3_tatoeba_ingest.py      # 采集语料
python phase3_day4_5_clean_validate.py    # 清洗 + 质量校验（需设 PYSPARK_PYTHON）
python phase3_vectorize.py                # 向量化（幂等，向量已存在则跳过）
python phase3_day6_7_faiss_search.py      # 语义检索

# —— 项目二：电商退款 ETL ——
python data/generate_refund_data.py               # 生成测试数据
python refund_etl.py --date 2025-06-01 --step all # 运行 ETL（可分步 extract / validate）
